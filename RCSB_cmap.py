...
)  # These will be saved in cmap.log
...
class CMAP:
    ...
    def __init__(self, structure, olig=None, chain=None, chains_like=None, leven=30):
        ...
        if chains_like:
            ...
            for chain in structure[0].get_chains():
                residues = Selection.unfold_entities(chain, "R")
                chain_info[chain] = sorted(
                    [(int(res._id[1]), res.resname) for res in residues],
                    key=lambda x: x[0],
                )
            for chain in chain_info:
                aa_dict[chain] = "".join(
                    [translate_aa[i[1]] for i in chain_info[chain]]
                )
            ...
            print()
            print(
                f"Chain IDs that were retained: {[key for key in structure[0].child_dict.keys()]}"
            )
            print()
    ...
    def Load_Data(self):
        ...
        for chain in self.structure[0].get_chains():
            ...
            for residue in residues:
                if (
                    residue.get_id()[1] > last_res + 1
                    and residue.get_id()[1] != residues[0].get_id()[1]
                ):
                    for idx in range(last_res + 1, residue.get_id()[1]):
                        ...
                        for atom in new_res:
                            atom.detach_parent()
                            atom.set_coord(
                                np.array([9999, 9999, 9999], dtype="float32")
                            )
    ...
    def levenshtein_dist_w_memory(self, string_1, string_2):
        ...
        leven_mtx = [
            [(0, "") for _ in range(len_string_2)] for _ in range(len_string_1)
        ]
    ...
    def Check_Length(self):
        ...
        if len(chain_ids) >= 2:
            ...
            def check_aa(domain):
                ...
                for chain in chain_info:
                    aa_mtx.append(
                        "".join([translate_aa[i[1]] for i in chain_info[chain]])
                    )
            ...
            for leven, chain in zip(ntd, self.structure[0].get_chains()):
                residues = sorted(
                    Selection.unfold_entities(chain, "R"), key=lambda x: x._id[1]
                )
                residue = residues[0]
    ...
    def Create_Map(self, cutoff):
        ...
        coor_sep = np.array(
            [[atom.coord for atom in chain[1]] for chain in chains.items()],
            dtype=object,
        )
        ...
        contact_map[(dist_matrix < cutoff) & (dist_matrix != 0.0)] = 1
        ...
        for key, idx in zip(combo, res_idx):
            ...
            if (
                key_a.parent is not None
                and key_b.parent is not None
                and idx[1] > idx[0]
            ):
                a = np.array(
                    [
                        key_a.parent.child_dict[key].coord
                        for key in key_a.parent.child_dict
                    ]
                )
                b = np.array(
                    [
                        key_b.parent.child_dict[key].coord
                        for key in key_b.parent.child_dict
                    ]
                )
        ...
        mask = (
            np.abs(
                np.arange(len(tot_idx_flat))
                - np.arange(len(tot_idx_flat))[:, np.newaxis]
            )
            <= 3
        )
        contact_map[mask] = 0
    ...
    def Collapse(self, cmap):
        ...
        if len(chain_ids) >= 2:
            ...
            inter_idx = [
                p
                for p in itertools.product(intra_idx, repeat=2)
            ...
            mask = (
                np.abs(
                    np.arange(mask.shape[0]) - np.arange(mask.shape[1])[:, np.newaxis]
                )
                <= 3
            )
            inter[mask] = 0
...
def Create_DF(
    ...
    name = (
        name.replace(re.search(r"(.cif|.pdb)", name).group(0), "")
        if bool(re.search(r"(.cif|.pdb)", name))
        else name
    )
...
def main(target, chain, olig, pdb2, chain2, chains_like, leven):
    if os.path.isfile(target):
        get_pdb = RCSB()
        structure_processed, aa_structure_processed = get_pdb.PDB_Processing(
            target, target[0:4], target[-3:]
        )
    else:
        get_pdb = RCSB()
        structure, format_pdb = get_pdb.Download_PDB(
            target, 1, chain
        )  # download assembly 0 from rcsb
        with tempfile.NamedTemporaryFile() as temp_file:
            if format_pdb == "pdb":
                temp_file.write(structure)
            else:
                with gzip.open(
                    io.BytesIO(structure), "rb"
                ) as gz:  # create an in-memory stream to read the gzipped data
                    gz_bytes = (
                        gz.read()
                    )  # .decode('utf-8')          #can decode to read string but write function is expecting bytes-like-object
            ...
            structure_processed, aa_structure_processed = get_pdb.PDB_Processing(
                temp_file.name, target, format_pdb
            )
    ...
    Cmap = CMAP(
        ...
        leven=leven,
    )  # cmap object now contains structural information!!!
    ...
    if olig:
        if len(structure_processed[0].child_list) > 1:
            ...
            Create_DF(
                cmap1_collapse, target, target, name=f"{target}_collapse", olig=olig
            )
        else:
    ...
    if pdb2:
        if os.path.isfile(pdb2):
            get_pdb = RCSB()
            structure_processed2, aa_structure_processed2 = get_pdb.PDB_Processing(
                pdb2, pdb2[0:4], pdb2[-3:]
            )
        else:
            get_pdb = RCSB()
            structure2, format_pdb2 = get_pdb.Download_PDB(
                pdb2, 1, chain2
            )  # download assembly 0 from rcsb
            with tempfile.NamedTemporaryFile() as temp_file:
                if format_pdb2 == "pdb":
                ...
                # remove hydrogens and non polymer atoms
                structure_processed2, aa_structure_processed2 = get_pdb.PDB_Processing(
                    temp_file.name, pdb2, format_pdb
                )

        Cmap2 = CMAP(
            structure_processed2, olig=olig, chain=chain2
        )  # cmap object now contains structural information!!!
        cmap2, mask2 = Cmap2.Create_Map(cutoff=8)  # cutoff in angstom

        Create_DF(cmap2, pdb2, pdb2, name=f"{pdb2}", mask=mask2)
        comp, x_name, padding_offset, y_name, align_offset, max_overlap = COMPARE(
            cmap1, cmap2, target, pdb2
        )
        ...
        if olig:
            if len(structure_processed2[0].child_list) > 1:
                ...
                Create_DF(
                    cmap2_collapse, pdb2, pdb2, name=f"{pdb2}_collapse", olig=olig
                )
            else:
                cmap2_collapse = cmap2
            comp, x_name, padding_offset, y_name, align_offset, max_overlap = COMPARE(
                cmap1_collapse, cmap2_collapse, target, pdb2
            )
...
"""
    ...
    if (
        not any((True if _ == "-pdb" else False for _ in sys.argv))
        or len(sys.argv) <= 2
    ):
        print(error_message)
        sys.exit()
    ...
    pdb2 = (
        None
        if not [idx for idx, _ in enumerate(sys.argv) if "-pdb2" == _]
        else sys.argv[[idx for idx, _ in enumerate(sys.argv) if "-pdb2" == _][0] + 1]
    )
    chain = (
        None
        if not [idx for idx, _ in enumerate(sys.argv) if "-chain" == _]
        else sys.argv[[idx for idx, _ in enumerate(sys.argv) if "-chain" == _][0] + 1]
    )
    chain2 = (
        None
        if not [idx for idx, _ in enumerate(sys.argv) if "-chain2" == _]
        else sys.argv[[idx for idx, _ in enumerate(sys.argv) if "-chain2" == _][0] + 1]
    )
    chains_like = (
        None
        if not [idx for idx, _ in enumerate(sys.argv) if "-chains_like" == _]
        else sys.argv[
            [idx for idx, _ in enumerate(sys.argv) if "-chains_like" == _][0] + 1
        ]
    )
    leven = (
        30
        if not [idx for idx, _ in enumerate(sys.argv) if "-leven" == _]
        else sys.argv[[idx for idx, _ in enumerate(sys.argv) if "-leven" == _][0] + 1]
    )
    olig = (
        True if any((True if _ == "-oligomer" else False for _ in sys.argv)) else False
    )
